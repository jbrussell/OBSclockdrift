function err = besselerr_joint(tw,xsp,varargin)
% Error function for fitting two nearby station pair's frequency domain xspectrum
% This version jointly inverts for the best fitting model of multiple cross
% spectra.
%
...
if ~iscell(xsp)
    error('Must use multiple cross spectra for this version of besselerr');
end
err = [];
for ii = 1:length(xsp)
    ...
    % % ADD FLATNESS
    err = [err; F1z(:); sm(:)*0.2*100; fl(:)*0.2*100; dx(:)*10*1];
end
